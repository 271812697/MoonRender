...
- 相机动画播放期间**鼠标输入会被整段跳过**，所以 `HandleInputs()` 里有三道保险，保证一个到不了的目标不会把相机锁死：按下中键/右键直接丢弃在飞的移动并接管相机；目标姿态非有限值（极点 fit 出来的 NaN）就丢弃该目标；插值一帧内毫无进展（float 精度到极限）就吸附到目标并结束（后两种都会打 `[Camera]` 日志）；
